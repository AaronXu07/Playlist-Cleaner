...
- [x] 1. Run database migrations in Supabase
...
- [x] 3. Create `src/lib/spotify.js` — Spotify HTTP client
  - [x] 3.1 Implement `refreshTokenIfNeeded(user)`
  ...
  - [x] 3.2 Implement `getCurrentlyPlaying(accessToken, userId)` with rate-limit retry loop
  ...
  - [x] 3.3 Implement `getRecentlyPlayed(accessToken, userId)` with rate-limit retry loop
  ...
  - [x] 3.4 Implement `removeTrackFromPlaylist(accessToken, playlistId, trackUri)`
...
- [x] 4. Checkpoint — Spotify client complete
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Create `src/lib/poller.js` — polling engine orchestrator
  - [x] 5.1 Set up module-level in-memory state and helpers
  ...
  - [x] 5.2 Implement `startPollingEngine()`
  ...
  - [x] 5.3 Implement `registerUser(userId)` and `deregisterUser(userId)`
  ...
  - [x] 5.4 Implement `runPollCycle(userId)` — guard, token refresh, and parallel fetch
  ...
  - [x] 5.5 Implement `processLiveTrack(userId, cpResult, state)` — Live_Track_Tracker
  ...
  - [x] 5.6 Implement `reconcileRecentlyPlayed(userId, rpItems, lastPollAt)` — Reconciler
  ...
  - [x] 5.7 Implement `writeListenEvent(event)` — Listen_Event_Writer
  ...
  - [x] 5.8 Implement `detectSkip(userId, trackId, playlistId)` — Skip_Detector + Track_Remover
  ...
  - [x] 5.9 Wire pipeline steps together inside `runPollCycle` and update `last_poll_at`
...
- [x] 6. Checkpoint — Core poller logic complete
  - Ensure all tests pass, ask the user if questions arise.


- [x] 7. Create `src/middleware/auth.js` — JWT cookie guard
  - [x] 7.1 Implement `requireAuth(req, res, next)` middleware
...
- [x] 8. Create `src/routes/api.js` — REST API endpoints
  - [x] 8.1 Implement `GET /api/removals` — list removal log for authenticated user
  ...
  - [x] 8.2 Implement `DELETE /api/removals/:id` — undo a removal
    - Apply `requireAuth` middleware
  ...
  - [x] 8.3 Implement `GET /api/events` — list recent listen events for authenticated user
    - Apply `requireAuth` middleware
    - Query `SELECT * FROM listen_events WHERE user_id = $req.user.userId ORDER BY listened_at DESC LIMIT 100`
    - Return JSON array; handle DB errors with 500
    - _Requirements: 5.1_

  - [x] 8.4 Implement `GET /api/status` — current polling state for authenticated user
    - Apply `requireAuth` middleware
...
- [x] 9. Wire polling engine into `src/index.js`
  - [x] 9.1 Import and call `startPollingEngine()` at server startup
  ...
  - [x] 9.2 Expose `registerUser` / `deregisterUser` to the auth callback for new users
...
- [x] 10. Checkpoint — Full pipeline wired, manual smoke-test ready
  - Ensure all tests pass, ask the user if questions arise.


- [x] 11. Write property-based tests for `src/lib/crypto.js`
  - [x] 11.1 Write property test for token encrypt/decrypt round-trip (Property 5)
...
- [x] 12. Write property-based tests for `src/lib/spotify.js`
  - [x] 12.1 Write property test for token refresh threshold (Property 4)
  ...
  - [x] 12.2 Write property test for Retry-After capping (Property 16)
    - File: `src/__tests__/spotify.test.js`
...
- [x] 13. Write property-based tests for `src/lib/poller.js` — polling loop properties
  - [x] 13.1 Write property test: only users with non-null refresh_token get intervals (Property 1)
  ...
  - [x] 13.2 Write property test: skip-if-running guard is concurrent-safe (Property 2)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 13.3 Write property test: stagger offset is always within [0, 5000] ms (Property 3)
    - File: `src/__tests__/poller.test.js`
...
- [x] 14. Write property-based tests for `src/lib/poller.js` — reconciler and listen event properties
  - [x] 14.1 Write property test: reconciler output newer than last_poll_at (Property 6)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 14.2 Write property test: reconciler playlist-context filter and dedup (Property 7)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 14.3 Write property test: listened_pct formula (Property 8)
    - File: `src/__tests__/poller.test.js`
...
- [x] 15. Write property-based tests for `src/lib/poller.js` — live track and event write properties
  - [x] 15.1 Write property test: max_progress_ms is running maximum (Property 9)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 15.2 Write property test: live event source is "delta" iff listened_pct < 0.50 (Property 10)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 15.3 Write property test: listen event writes are idempotent (Property 11)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 15.4 Write property test: was_skipped = (listened_pct < 0.25) (Property 12)
    - File: `src/__tests__/poller.test.js`
    - Use `fc.float({ min: 0, max: 1 })` for `listened_pct`
...
- [x] 16. Write property-based tests for `src/lib/poller.js` — skip detection and engine-level properties
  - [x] 16.1 Write property test: skip detection trigger condition (Property 13)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 16.2 Write property test: re-add history cutoff (Property 14)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 16.3 Write property test: last_poll_at always updated to cycleStart (Property 15)
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 16.4 Write property test: Reduced_Interval_Mode transitions (Property 17)
    - File: `src/__tests__/poller.test.js`
...
- [x] 17. Checkpoint — All property tests written
  - Ensure all tests pass, ask the user if questions arise.


- [x] 18. Write integration and unit tests
  - [x] 18.1 Write unit tests for token refresh scenarios
    - File: `src/__tests__/spotify.test.js`
  ...
  - [x] 18.2 Write unit tests for Live_Track_Tracker state transitions
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 18.3 Write unit tests for revoked permissions flow
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 18.4 Write unit tests for 403 Spotify-owned playlist handling
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 18.5 Write integration test: full poll cycle with mocked Spotify client
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 18.6 Write integration test: skip detection → removal log written
    - File: `src/__tests__/poller.test.js`
  ...
  - [x] 18.7 Write integration test: `registerUser` post-startup via auth callback
    - File: `src/__tests__/poller.test.js`
